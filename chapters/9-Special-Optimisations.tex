...
In the full argument, prover and verifier recursively run the reduction to obtain increasingly smaller statements. The argument is then concluded with the prover revealing a %and at the end the prover simply reveals a 
...
Furthermore, even if the prover is dishonest, we can show that if the prover can open $A'$ with respect to the key $\vec{g}'$ for $2m-1$ different challenges, then we can extract opening $(\vec{a}_1,\ldots,\vec{a}_m)$ corresponding to $A=\prod_{i=1}^m\vec{g}_i^{\vec{a}_i}$. %and that the opening of $A'$ must be $\vec{a}'$.
